...
def favorable_categories_of_increased_cashback(year: str, month: str, data: pd.DataFrame) -> str:
    ...
    try:
        ...
        date_obj_end = datetime.datetime(
            int(year), int(month), monthrange(int(year), int(month))[1], 23, 59, 59
        ).date()
    except ValueError:
        logger_serv.error("Некорректный формат даты")
        return ""
    df_copy = data[["Дата операции", "Сумма операции", "Статус", "Кэшбэк", "Категория"]].copy()
